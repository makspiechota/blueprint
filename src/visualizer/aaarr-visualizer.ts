...
function formatMetricValue(value: any): string {
  ...
  if (value.amount !== undefined && value.currency) {
    ...
    const symbol = currencySymbols[value.currency] || value.currency;
      ...
      ${viability.targets ? `
        <div class="viability-section">
          ...
          ${Object.entries(viability.targets).map(([key, target]: [string, any]) => {
            ...
            return entries.map(([subKey, subTarget]: [string, any]) => `
              <div id="lean-viability-targets-${key}-${subKey}" class="viability-field">
                <span class="field-label">${key.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase())} ${subKey.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase())}:</span>
                <span class="field-value">
                  ${subTarget.rate !== undefined ? `${subTarget.rate}/${subTarget.period}` : ''}
                  ${subTarget.amount !== undefined ? `${subTarget.currency} ${subTarget.amount.toLocaleString()}` : ''}
                  ${subTarget.percentage !== undefined ? `${subTarget.percentage}%` : ''}
                </span>
              </div>
            `).join('');
          }).join('')}
